...
from pruning_utils.group_lasso import group_lasso_regularize, print_params_sparsity, print_params_block_sparsity
...
def take_optimizer_step(args, optimizer, model, overflow_buf, global_step, params, whole_params, params_masks, writer):
    ...
    if args.allreduce_post_accumulation:
        ...
        if had_overflow == 0:
            #============================================
            # TODO: parse whole_params and params_masks
            lr = optimizer.param_groups[0]['lr']
            alpha = lr / args.learning_rate * 0.00015
            # alpha = 0.00015
            # if len(params) == 0:
            #     prune_block_gradients(whole_params, params_masks)
            # else:
            group_lasso_regularize(params, [2, 1], params_masks, alpha)
            prune_gradients(whole_params, params_masks)
            #============================================
            optimizer.step()
            #============================================
            # if len(params) != 0:
            prune_weights(whole_params, params_masks)
            params = print_params_block_sparsity(whole_params, [2, 1], global_step, pruning_threshold=1e-4)
            params_masks = create_block_masks(whole_params, params, params_masks, [2, 1])
            #============================================
            global_step += 1
        else:
...
# Sparse functions
# ========================
def prune_gradients(params, block_masks):
    for n, p in params:
        if n not in block_masks.keys():
            p.grad.data[p.data==0.0] = 0.0
        else:
            p.grad.data *= block_masks[n]

def prune_block_gradients(params, block_masks):
    for n, p in params:
        if n in block_masks.keys():
            p.grad.data *= block_masks[n]

def create_block_masks(whole_params, mlp_params, block_masks, block_size):
    if len(whole_params) == len(block_masks):
        return block_masks
    keys = []
    for n, p in mlp_params:
        keys.append(n)
    for n, p in whole_params:
        if n not in keys:
            shape = p.shape
            new_shape = [shape[0] // block_size[0], block_size[0], shape[1] // block_size[1], block_size[1]]
            param = p.reshape(new_shape)
            normed = torch.norm(param, p = 1, dim = [1, 3])
            normed[normed!=0.0] = 1.0
            normed = normed.repeat_interleave(block_size[0], dim=0).repeat_interleave(block_size[1], dim=-1)
            block_masks[n] = normed
    return block_masks

def prune_weights(params, block_mask):
    thr = 1e-4
    for n, p in params:
        if n not in block_mask.keys():
            p.data[torch.abs(p.data) < thr] = 0.0
        else:
            p.data *= block_mask[n]

# def prune_gradients(params):
#     for n, p in params:
#         p.grad.data[p.data==0.0] = 0.0
# 
# def prune_block_gradients(params, block_masks):
#     if len(params) == len(block_masks):
#         for i in range(len(params)):
#             params[i][1].grad.data *= block_masks[i]
# 
# def create_block_masks(params, block_size):
#     block_masks = []
#     for i in range(len(params)):
#         shape = params[i][1].shape
#         new_shape = [shape[0] // block_size[0], block_size[0], shape[1] // block_size[1], block_size[1]]
#         param = params[i][1].reshape(new_shape)
#         normed = torch.norm(param, p = 1, dim = [1, 3])
#         normed[normed!=0.0] = 1.0
#         normed = normed.repeat_interleave(block_size[0], dim=0).repeat_interleave(block_size[1], dim=-1)
#         block_masks.append(normed)
#     return block_masks
# 
...
#        p.data[torch.abs(p.data) < thr] = 0.0
# 
# def params_block_sparsity_writer(params, block_size, writer, epoch, pruning_threshold=0):
#     sparsity_dict = {}
#     for i in range(len(params)):
#         key = params[i][0]
#         param = params[i][1]
...
#     writer.add_scalars("sparsity for block size {}".format(block_size), sparsity_dict, epoch)
...
def main():
    ...
    writer = SummaryWriter(comment=args.bert_model)
    #============================================
    params = [(n, p) for (n, p) in model.named_parameters() if "bias" not in n and "LayerNorm" not in n and "classifier" not in n and "embeddings" not in n]
    whole_params = [(n, p) for (n, p) in model.named_parameters() if "bias" not in n and "LayerNorm" not in n and "classifier" not in n and "embeddings" not in n]
    params_masks = {}
    is_masks_set = False
    global_step = 0
    #============================================
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if training_steps % args.gradient_accumulation_steps == 0:
                        ...
                        global_step = take_optimizer_step(args, optimizer, model, overflow_buf, global_step, params, whole_params, params_masks, writer)
                        # #============================================
                        # if len(params) == 0 and is_masks_set == False:
                        #     params_masks = create_block_masks(whole_params, [2, 1])
                        #     is_masks_set = True
                        # else:
                        #     is_masks_set = False
                        #     params_masks = []
